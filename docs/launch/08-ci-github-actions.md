...
**Was:** `ios/Runner.xcodeproj/project.pbxproj` contained 11 lines referring to `Flutter/Release/{App,Flutter}.xcframework` — files that don't exist on fresh checkouts (the `Release/` dir is gitignored and regenerated by `flutter build ios --release` locally). On CI these references caused the iOS build to fail with `'There is no XCFramework found at .../ios/Flutter/Release/App.xcframework'`. The job ran with `continue-on-error: true` to keep the overall workflow green while analyze + test + Android were the real gates.

**Fix:** removed all spurious entries by comparing against a fresh `flutter create` scaffold (which contains zero `Flutter/Release/` references). Specifically:
...
Verified locally with `flutter build ios --debug --no-codesign` — `✓ Built build/ios/iphoneos/Runner.app`.

`continue-on-error: true` removed from `build-ios` job in `.github/workflows/ci.yml`. The iOS build is now a real gate alongside Android and tests.

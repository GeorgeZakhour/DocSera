...
| **Encryption** | `MessageEncryptionService` — GCM round-trips with the `"ENCv2:"` prefix, AEAD tamper-rejection (wrong key + bit-flipped ciphertext), missing-key graceful degradation, **plus backward-compat decryption of legacy `"ENC:"` (CBC + PKCS7) blobs**. | Healthtech encryption regression = privacy breach. Must be bullet-tested. |

...
| 9b | iOS pbxproj cleanup — removed 11 spurious lines referring to `Flutter/Release/{App,Flutter}.xcframework` from `ios/Runner.xcodeproj/project.pbxproj` (3 PBXBuildFile + 4 PBXFileReference + 4 references across PBXFrameworksBuildPhase, PBXCopyFilesBuildPhase, and 2 PBXGroups). Verified locally with `flutter build ios --debug --no-codesign`. CI's `build-ios` job flipped from `continue-on-error: true` to a real gate. | [08-ci-github-actions.md](08-ci-github-actions.md) | 9.43 |

...
import 'add_family_bottom_sheet.dart';
          ...
          floatingActionButton: BlocBuilder<HealthCubit, HealthState>(
            builder: (context, state) {
              ...
              return FloatingActionButton.extended(
                ...
                icon: Icon(Icons.group_rounded, size: 20.sp, color: Colors.white),
                label: Text(
                  t.family_add_button,
            ...
            child: Padding(
              ...
              child: BlocBuilder<HealthCubit, HealthState>(
                builder: (context, state) {
                  ...
                  // EMPTY — DECLARED NO FAMILY HISTORY
                  if (state.records.isEmpty && state.noItemsDeclared) {
                    return HealthNoItemsView(
                      icon: Icons.check_circle_rounded,
                      title: t.family_no_records_title,
                      subtitle: t.family_no_records_subtitle,
                      changeDecisionText: t.family_no_records_change,
                      onChangeDecision: () {
                        context.read<HealthCubit>().setNoItemsDeclared(false);
                      },
                      addButtonText: t.family_no_records_add,
                      onAddPressed: () {
                        final cubit = context.read<HealthCubit>();
                        _openAddBottomSheet(context, cubit);
                      },
    ...
    showModalBottomSheet(
      ...
      builder: (_) {
        return BlocProvider.value(
          value: cubit,
          child: const AddFamilyBottomSheet(),
        );
      },
    );
  }
...
class _FamilyList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        // --------- READ FAMILY MEMBERS + AGE FROM NOTES ---------
        final notes = _parseNotes(record);
        final member = (notes["family_members"] as List?)?.first ?? "";
        final age = notes["diagnosis_age"];
        ...
        return HealthRecordCard(
          ...
          tags: [
            ...
            if (member.isNotEmpty)
              _tag(member, AppColors.main),

            // DIAGNOSIS AGE TAG
            if (age != null)
              _tag("${t.family_age_at_diagnosis}: $age", Colors.blueGrey),
  ...
  void _openDetails(BuildContext context, HealthRecord record) {
    ...
    // READ NOTES JSON
    final notes = _parseNotes(record);
    final member = (notes["family_members"] as List?)?.first ?? t.notProvided;
    final age = notes["diagnosis_age"] ?? t.notProvided;
    ...
    showDialog(
      ...
      builder: (_) => HealthRecordDetailsDialog(
        ...
        rows: [
          DetailRow(t.family_condition, name),
          if (desc.isNotEmpty) DetailRow(t.description, desc),

          // NEW ROWS
          DetailRow(t.family_member, member),
          DetailRow(t.family_age_at_diagnosis, age.toString()),
  ...
  void _openDelete(BuildContext context, String id) {
    ...
    showDialog(
      ...
      builder: (_) => HealthDeleteConfirmDialog(
        title: t.deleteTheFamily,
        message: t.areYouSureToDeleteFamily,
        deleteText: t.delete,
        cancelText: t.cancel,
        onConfirm: () {
          Navigator.pop(context);

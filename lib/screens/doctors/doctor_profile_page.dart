...
class _DoctorProfilePageState extends State<DoctorProfilePage> {
  ...
  late StreamSubscription<DocumentSnapshot> _doctorListener;
  ...
  @override
  void dispose() {
    _doctorListener.cancel(); // ✅ Stop listening to Firestore updates
  ...
  Future<void> _loadDoctorProfile() async {
    ...
    if (widget.doctor != null && widget.doctor!.isNotEmpty) {
      ...
      if (cachedData != null) {
        ...
        print("⚡ Loaded Cached Doctor Data: ${cachedDoctor['firstName']} ${cachedDoctor['lastName']} (SECOND CASE)");

        setState(() {
          _doctorData = {...cachedDoctor}; // Assign to local state
        });
      }
    }

    // ✅ Set up Firestore real-time listener
    _doctorListener = FirebaseFirestore.instance
        .collection('doctors')
        .doc(doctorId)
        .snapshots()
        .listen((DocumentSnapshot snapshot) async {
      if (snapshot.exists) {
        Map<String, dynamic> firestoreDoctor = snapshot.data() as Map<String, dynamic>;
        print("📡 Real-Time Update from Firestore for Doctor: ${firestoreDoctor['firstName']} ${firestoreDoctor['lastName']} (THIRD CASE)");

        // ✅ Convert Timestamps Before Caching
        convertTimestamps(firestoreDoctor);

        // ✅ Cache the new data (after converting Timestamps)
        final SharedPreferences prefs = await SharedPreferences.getInstance();
        await prefs.setString('doctor_$doctorId', json.encode(firestoreDoctor));

        // ✅ Update UI
        setState(() {
          _doctorData = {...firestoreDoctor}; // Replaces the data
        });
...
// ✅ Share Doctor Profile Function
  void _shareDoctorProfile() {
    ...
    String doctorName = "${_doctorData?['firstName']} ${_doctorData?['lastName']}";
...
🏥 العيادة: $clinic
📍 العنوان: $fullAddress
📞 الاتصال: ${_doctorData?['phoneNumber'] ?? AppLocalizations.of(context)!.notProvided}
...
🏥 Clinic: $clinic
📍 Address: $fullAddress
📞 Contact: ${_doctorData?['phoneNumber'] ?? AppLocalizations.of(context)!.notProvided}
...
// ✅ Address Line 2 (City, Country)
  ...
  void _toggleFavoriteStatus() async {
    ...
    try {
      ...
      if (!userDoc.exists || !userDoc.data().toString().contains('favorites')) {
        await userRef.set({'favorites': []}, SetOptions(merge: true));
        print("📄 Created 'favorites' field for user.");
      }
  ...
  Widget build(BuildContext context) {
    ...
    final String? profileDescription = doctor['profileDescription'];
            ...
            slivers: [
              ...
              SliverAppBar(
                ...
                flexibleSpace: FlexibleSpaceBar(
                  title: Opacity(
                    ...
                    child: Text(
                      "${_doctorData?['title'] ?? ''} ${_doctorData?['firstName'] ?? ''} ${_doctorData?['lastName'] ?? ''}".trim(),
              ...
              SliverPersistentHeader(
                ...
                delegate: _ButtonHeaderDelegate(
                  ...
                  _doctorData?['title'] ?? '',
                  _doctorData?['gender'] ?? '',
                  _doctorData?['firstName'] ?? '',
                  _doctorData?['lastName'] ?? '',
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        SizedBox(height: 20.h),
                        if (_doctorData?['openingHours'] != null || _doctorData?['languages'] != null || _doctorData?['phoneNumber'] != null)
                          _buildInfoSection(
                            _doctorData?['phoneNumber']?.toString(),
                            _doctorData?['openingHours'] ?? {},

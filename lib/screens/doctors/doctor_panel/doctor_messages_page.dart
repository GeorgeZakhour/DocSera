...
class DoctorMessagesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //         };
      //         final unread = data['unreadCountForDoctor'] ?? 0;
      //
      //         return {
      //           'doc': doc,
      //           'lastMessage': lastMessage,
      //           'lastTimestamp': (data['updatedAt'] as Timestamp).toDate(),
      //           'unread': unread,
      //         };
      //       })),
      //
      ...
      //
      //         final sorted = snap.data!..sort((a, b) => (b['lastTimestamp'] as DateTime).compareTo(a['lastTimestamp'] as DateTime));
      //         final grouped = <int, List<Map<String, dynamic>>>{};
      //         for (var item in sorted) {
      //           final year = (item['lastTimestamp'] as DateTime).year;
      ...
      //                   final accountHolderName = data['accountHolderName'] ?? '';
      //                   final unreadCount = item['unread'] ?? 0;
      //                   final lastText = lastMessage?['text'] ?? data['lastMessage'] ?? '';
      //                   final timestamp = item['lastTimestamp'] as DateTime;
      //                   final now = DateTime.now();
      //                   final isToday = now.year == timestamp.year &&
      //                       now.month == timestamp.month &&
      //                       now.day == timestamp.day;
      //                   final isYesterday = now.subtract(const Duration(days: 1)).year == timestamp.year &&
      ...
      //                       InkWell(
      //                         onTap: () {
      //                           final fullName = '${doctorData?['title'] ?? ''} ${doctorData?['firstName'] ?? ''} ${doctorData?['lastName'] ?? ''}'.trim();

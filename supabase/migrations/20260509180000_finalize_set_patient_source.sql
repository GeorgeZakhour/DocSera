-- Patch fn_cron_account_deletion_finalize: also flip the patient_source
-- discriminator from 'user' to 'manual' on every clinical record we
-- re-point. Three tables carry this column (accounting, dental_records,
-- reports) — Pro's section queries filter by patient_source so leaving
-- it at 'user' makes the rows invisible to the manual-patient view
-- even though patient_id has been updated.

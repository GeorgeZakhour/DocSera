-- Fix duplicated "د. " prefix in fn_cron_message_long_unread title.
-- conversations.doctor_name already carries "د. " in Arabic, so adding
-- another in the template produces "د. د. {name}". Same fix used by
-- handlers/conversations.ts on the edge-function side. Strip any leading
-- "د. " from the source name before composing the AR title; the EN title
-- continues to prefix "Dr." after stripping the AR prefix from the same
-- source string.
...
BEGIN
  ...
  LOOP
    ...
    -- Strip any leading "د. " — doctor_name already carries the prefix.
    v_name := COALESCE(NULLIF(regexp_replace(r.doctor_name, '^د\.\s*', ''), ''), '');

    IF v_locale = 'en' THEN
      v_title := 'Unread message from Dr. ' || COALESCE(NULLIF(v_name, ''), 'your doctor');
      v_body  := 'You have a message you haven''t read in ' || floor(r.hours_old/24)::text || ' days.';
    ELSE
      v_title := 'رسالة غير مقروءة من د. ' || COALESCE(NULLIF(v_name, ''), 'الطبيب');
